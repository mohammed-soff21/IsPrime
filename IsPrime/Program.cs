namespace IsPrime
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                // Welcome message
                WelcomeApp("checking if the number is prime or not");
                // To read user input and validate it
                do
                {
                    if (IsNumber("a number to check", out nUserInput))
                        break;
                } while (true);
                if(!IsPrime(nUserInput))
                    Print($"The number {nUserInput} is not a prime number");
                else
                    Print($"The number {nUserInput} is a prime number");
        ...
        // 7) this method to check if the number is prime or not
